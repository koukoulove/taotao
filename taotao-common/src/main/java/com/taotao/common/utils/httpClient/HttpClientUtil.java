...
public class HttpClientUtil{
	...
	static{
		try {
		        System.out.println("------ HttpClientUtil init begin-------");
			client4HTTP = HCB.custom().build();
			System.out.println("client4HTTP=="+client4HTTP);
			client4HTTPS = HCB.custom().ssl().build();
			System.out.println("client4HTTPS=="+client4HTTPS);
			System.out.println("------ HttpClientUtil init end-------");
	...
	private static HttpResponse execute(HttpConfig config) throws HttpProcessException {
		...
		try {
			...
			//执行请求操作，并拿到结果（同步阻塞）
			System.out.println("config.client()=="+config.client());
